...
{
    [DesignerGenerated]
	public class frmPDELOTDSP : Form
	{
		...
		public frmPDELOTDSP()
		{
			base.Shown += frmPDELOTDSP_Shown;
			base.FormClosing += frmPDELOTDSP_FormClosing;
			InitializeComponent();
		}
		...
		private void InitializeComponent()
		{
			...
			this.TableLayoutPanel3.Controls.Add(this.cbREVNO, 3, 1);
			...
			this.btnRetrieve.Location = new System.Drawing.Point(705, 2);
			...
			this.btnClose.Location = new System.Drawing.Point(831, 2);
			...
			this.cbREVNO.Items.AddRange(new object[3] { "In used Only", "Disposed Only", "All" });
			this.cbREVNO.Location = new System.Drawing.Point(543, 37);
			this.cbREVNO.Name = "cbREVNO";
			this.cbREVNO.Size = new System.Drawing.Size(156, 28);
			this.cbREVNO.TabIndex = 84;
		...
		private void frmPDELOTDSP_Shown(object sender, EventArgs e)
		{
		...
		private void frmPDELOTDSP_FormClosing(object sender, FormClosingEventArgs e)
		{
			string strSQL = "DELETE PDFGR_WRK WHERE PDFGR_PRNID = '" + MyProject.Computer.Name + "'";
			DB.ExecProc(strSQL);
		...
		private void BindComboBox(string strOption)
		{
			...
			{
				cbITMCD.DataTable = "PDFGR_TBL WITH (NOLOCK) ";
				cbITMCD.WhereClause = "PDFGR_LOCID = '" + Common.gfValidSQLStr(Conversions.ToString(cbLOCID.SelectedValue)) + "'";
				cbITMCD.ValueMember = "PDFGR_MDLCD";
				cbITMCD.DisplayMember = "PDFGR_MDLCD";
				cbITMCD.LoadData();
			}
			if (Operators.CompareString(strOption, "REVNO", false) == 0)
			{
				strSQL = "SELECT DISTINCT PDFGR_REVNO ";
				strSQL += "FROM PDFGR_TBL WITH (NOLOCK) ";
				strSQL = strSQL + "WHERE PDFGR_LOCID = '" + Common.gfValidSQLStr(Conversions.ToString(cbLOCID.SelectedValue)) + "' ";
				strSQL = strSQL + "AND PDFGR_MDLCD = '" + Common.gfValidSQLStr(Conversions.ToString(cbITMCD.SelectedValue)) + "' ";
				strSQL += "ORDER BY PDFGR_REVNO ";
				cbREVNO.ValueMember = "PDFGR_REVNO";
				cbREVNO.DisplayMember = "PDFGR_REVNO";
		...
		{
			string strSQL = "EXEC spPDELOT_WRK '" + MyProject.Computer.Name + "', ";
		...
		private void RefreshGrid()
		{
			...
			string strSQL = "EXEC spPDLOT '" + MyProject.Computer.Name + "', " + Conversions.ToString(cboSTSFG.SelectedIndex);
		...
		private void txtGRLNO_KeyDown(object sender, KeyEventArgs e)
		{
			...
			{
				string strSQL = "SELECT * FROM PDFGR_TBL WITH (NOLOCK) WHERE PDFGR_FGLNO = '" + Common.gfValidSQLStr(txtGRLNO.Text) + "'";
				DataTable dt = DB.ExecProc(strSQL);
				if (dt.Rows.Count == 0)
				{
				...
				else if (Operators.ConditionalCompareObjectEqual(dt.Rows[0]["PDFGR_STSFG"], 3, false))
				{
