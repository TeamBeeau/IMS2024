...
namespace IMS
{
	...
	public class frmPDERWO : Form
	{
		...
		internal virtual ComboBox cbType
		{
			...
			set
			{
				EventHandler value2 = cbType_SelectedIndexChanged;
				ComboBox comboBox = _cbType;
				if (comboBox != null)
				{
					comboBox.SelectedIndexChanged -= value2;
				}
		...
		public frmPDERWO()
		{
			base.FormClosing += frmPDERWO_FormClosing;
			base.Shown += frmPDERWO_Shown;
			InitializeComponent();
		}
		...
		private void InitializeComponent()
		{
			...
			this.TableLayoutPanel1.Controls.Add(this.cbType, 5, 1);
			...
			this.txtSTSFG.Location = new System.Drawing.Point(745, 59);
			...
			this.cbType.Location = new System.Drawing.Point(566, 29);
			this.cbType.Name = "cbType";
			this.cbType.Size = new System.Drawing.Size(173, 24);
			this.cbType.TabIndex = 93;
			...
			this.Label3.Text = "Work Order Type";
			...
			this.numWOQTY.Location = new System.Drawing.Point(746, 539);
			...
			base.Name = "frmPDERWO";
			base.Tag = "PDERWO";
			this.Text = "Customer Return Work Order";
		...
		private void frmPDERWO_FormClosing(object sender, FormClosingEventArgs e)
		{
		...
		{
			if (Operators.CompareString(strOption, "", false) == 0)
			{
				...
				strSQL += "WHERE MACOD_CTYID = 'CRTYPE' ";
			...
			{
				cbMDLCD.DataTable = "PCRTN_TBL WITH (NOLOCK) ";
				cbMDLCD.WhereClause = Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("PCRTN_LOCID = '", cbLOCID.SelectedValue), "'"));
				cbMDLCD.ValueMember = "PCRTN_ITMCD";
				cbMDLCD.DisplayMember = "PCRTN_ITMCD";
				cbMDLCD.LoadData();
			}
			if (Operators.CompareString(strOption, "REVNO", false) == 0)
			{
				string strSQL = "SELECT DISTINCT PCRTN_REVNO ";
				strSQL += "FROM PCRTN_TBL WITH (NOLOCK) ";
				strSQL = Conversions.ToString(Operators.ConcatenateObject(strSQL, Operators.ConcatenateObject(Operators.ConcatenateObject("WHERE PCRTN_LOCID = '", cbLOCID.SelectedValue), "' ")));
				strSQL = Conversions.ToString(Operators.ConcatenateObject(strSQL, Operators.ConcatenateObject(Operators.ConcatenateObject("AND PCRTN_ITMCD = '", cbMDLCD.SelectedValue), "'")));
				cbREVNO.ValueMember = "PCRTN_REVNO";
				cbREVNO.DisplayMember = "PCRTN_REVNO";
		...
		private void frmPDERWO_Shown(object sender, EventArgs e)
		{
		...
		private void BindGridHeader()
		{
			...
			cbType.SelectedValue = RuntimeHelpers.GetObjectValue(dgvHEADER.CurrentRow.Cells["PDRWO_TYPE"].Value);
			...
			BindGrid(Conversions.ToString(cbType.SelectedValue));
			EnableControl();
		}
		...
		private void EnableControl()
		{
			if (Conversions.ToDouble(txtSTSFG.Text) == 1.0)
			{
                ...
                btnREMOVE.Enabled = Common.GetAccessLevel(Conversions.ToString(base.Tag)) >= 3;
		...
		private void BindGrid(String Type)
		{
			...
			string strSQL = "EXEC spPDRWO_Grid '" + Common.gfValidSQLStr(MyProject.Computer.Name) + "', ";
			...
			strSQL = strSQL + "'" + Common.gfValidSQLStr(Type) + "', ";
			...
			checked
			{
				if (dt.Rows.Count > 0)
				{
					...
					dataGridView.Columns[i].ReadOnly = true;
					i++;
					dataGridView.Columns[i].Visible = Conversions.ToBoolean(Interaction.IIf(Operators.ConditionalCompareObjectEqual(cbType.SelectedValue, "REMIX", false), true, false));
					dataGridView.Columns[i].Width = 150;
		...
		private void cbMDLCD_SelectedIndexChanged(object sender, EventArgs e)
		{
            ...
            BindGrid(Conversions.ToString("REMIX"));
            RecalculateWOQty();
        }
		...
		{
			GenerateQRCode();
			object selectedValue = cbType.SelectedValue;
			if (Operators.ConditionalCompareObjectEqual(selectedValue, "REPAIR", false))
			{
				DisplayReport();
			}
			else if (Operators.ConditionalCompareObjectEqual(selectedValue, "REMIX", false))
			{
				DisplayLabel();
			}
		}
		...
		{
			lblMDLCD.Visible = true;
			cbMDLCD.Visible = true;
			lblREVNO.Visible = true;
			cbREVNO.Visible = true;
			if (Operators.ConditionalCompareObjectEqual(cbType.SelectedValue, "REMIX", false))
			{
				lblMDLCD.Visible = false;
				cbMDLCD.Visible = false;
