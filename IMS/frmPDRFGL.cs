...
namespace IMS
{
	...
	public class frmPDRFGL : Form
	{
		...
		public frmPDRFGL()
		{
			base.Shown += frmPDRFGL_Shown;
			base.FormClosing += frmPDRFGL_FormClosing;
			InitializeComponent();
		}
		...
		private void InitializeComponent()
		{
			...
			this.TableLayoutPanel3.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle());
			this.TableLayoutPanel3.Controls.Add(this.btnEXCEL, 0, 2);
			...
			this.TableLayoutPanel3.Controls.Add(this.btnClose, 2, 2);
			...
			this.btnEXCEL.Location = new System.Drawing.Point(129, 50);
			...
			this.btnClose.Location = new System.Drawing.Point(255, 50);
		...
		private void frmPDRFGL_Shown(object sender, EventArgs e)
		{
		...
		private void btnPRINT_Click(object sender, EventArgs e)
		{
			...
			frmRPT.ReportName = "Finished Goods Ledger";
			frmRPT.ReportFile = PublicVar.gstrRptPath + "crptPDRFGL.rpt";
			frmRPT.Params = "@FRDATE=" + Common.gfSQLDate(dtpFRDATE.Value) + ",@TODATE=" + Common.gfSQLDate(dtpTODATE.Value);
			frmRPT.Formulas = ("COMNM='" + Common.gfValidSQLStr(PublicVar.gstrCompany.Replace(",", "")) + "',RPTTITLE='Finished Goods Ledger',DTFMT=" + Conversions.ToString(PublicVar.gintRptDateFmt) + ",QTYDS=" + PublicVar.gintQTYDS + ",AMTDS=" + PublicVar.gintAMTDS + ",PRCDS=" + PublicVar.gintPRCDS + ",WGTDS=" + PublicVar.gintWGTDS) ?? "";
		...
		private void frmPDRFGL_FormClosing(object sender, FormClosingEventArgs e)
		{
		...
		private void bgwProgress_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			xlsApp.Cells[2, 1] = Common.gfConvertLanguage(PublicVar.gstrLanguage, Conversions.ToString(base.Tag), "Finished Goods Ledger");
			...
			xlsApp.Cells[intRow, 15] = Common.gfConvertLanguage(PublicVar.gstrLanguage, Conversions.ToString(base.Tag), "Order Type");
			((_Worksheet)xlsSheet).get_Range(RuntimeHelpers.GetObjectValue(xlsSheet.Cells[1, 1]), RuntimeHelpers.GetObjectValue(xlsSheet.Cells[5, 15])).Font.Bold = true;
			...
			((_Worksheet)xlsSheet).get_Range(RuntimeHelpers.GetObjectValue(xlsSheet.Cells[1, 15]), RuntimeHelpers.GetObjectValue(xlsSheet.Cells[1, 15])).EntireColumn.NumberFormat = "@";
			...
			checked
			{
				...
				{
					intRow++;
					xlsApp.Cells[intRow, 1] = RuntimeHelpers.GetObjectValue(dt.Rows[i]["SO_DELDT"]);
					...
					xlsApp.Cells[intRow, 15] = RuntimeHelpers.GetObjectValue(dt.Rows[i]["ORDER_TYPE"]);
